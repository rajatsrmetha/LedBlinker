...
## Building and Running the LED Application

In order to build the LED application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the LED application's code.
```
fprime-util build
```
...
To run the ground system without starting the LED app:
```
...
```
cd LED/build-artifacts/<platform>/bin/
./LED -a 127.0.0.1 -p 50000
```
